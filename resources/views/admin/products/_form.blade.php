...
<div class="form-group mb-3">
    ...
    <div>
        <label class="me-2">
            <input type="radio" name="status" value="in-stock" @if (old('status', $product->status) == 'in-stock') checked @endif >
            In Stock
        </label>
        <label class="me-2">
            <input type="radio" name="status" value="sold-out" @if (old('status', $product->status) == 'sold-out') checked @endif >
            Sold Out
        </label>
        <label>
            <input type="radio" name="status" value="draft" @if (old('status', $product->status) == 'draft') checked @endif >
            Draft
        </label>
    </div>
    @error('status')
...
<div class="form-group mb-3">
    <button class="btn btn-primary" type="submit">{{ $button_lable ?? 'Save' }}</button>
</div>
